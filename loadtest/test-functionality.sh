...
echo -e "${BLUE}🧪 Artillery負荷テスト機能テスト開始${NC}"
echo "========================================"
...
# Artilleryプロセッサーの構文チェック
for processor_file in "$SCRIPT_DIR/artillery"/*.js; do
    if [[ -f "$processor_file" ]]; then
        filename=$(basename "$processor_file")
        if node -c "$processor_file" > /dev/null 2>&1; then
...
echo ""
echo -e "${BLUE}📝 使用例:${NC}"
echo "  ./run-tests.sh --max-users 100 optimized"
echo "  ./run-tests.sh -e local -u 50 artillery"
